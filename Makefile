...
#   make ci                        # lint + static + tests + coverage
#
# Maintenance
...
help:
	...
	@echo "  ci                              Run lint + static + tests + coverage"
...
static-phpstan-tests:
	@if [ -z "$$(find tests -type f -name '*.php' 2>/dev/null)" ]; then \
		...
		$(PHP) -d memory_limit=$(PHP_MEMORY_LIMIT) $(PHPSTAN) analyse -c phpstan.tests.neon $(PHPSTAN_TESTS_PARAMS); \
	fi
