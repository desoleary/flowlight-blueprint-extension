...
describe('output', function () {
    it('generates output with configured generator', function () {
        ...
        $gen = makeApiGenWithConfig($this->files, [
            'dto' => ['class' => $genClass, 'stub' => 'dto.stub', 'config_class' => $cfgClass],
        ]);

        $this->files->shouldReceive('exists')->withArgs(fn ($p) => str_contains($p, 'stubs/flowlight/dto.stub'))->andReturn(false);
        $this->files->shouldReceive('exists')->withArgs(fn ($p) => str_contains($p, '/stubs/dto.stub'))->andReturn(true);
        $this->files->shouldReceive('get')->withArgs(fn ($p) => str_contains($p, '/stubs/dto.stub'))->andReturn('// default dto stub');
...
describe('requireConfigValue', function () {
    it('throws when class key is missing', function () {
        ...
        $expect = fn () => makeApiGenWithConfig($this->files, [
            'dto' => ['stub' => 'dto.stub', 'config_class' => $cfgClass],
        ]);
    ...
    it('throws when config_class key is missing', function () {
        $genClass = makeAnonGeneratorClass($this->files);
        $expect = fn () => makeApiGenWithConfig($this->files, [
            'dto' => ['class' => $genClass, 'stub' => 'dto.stub'],
        ]);
        expect($expect)->toThrow(LogicException::class);
    });
});
...
describe('resolveStub', function () {
    ...
    it('returns the default stub when custom override does not exist', function () {
        $probe = makeApiGenProbe($this->files, ['dto' => ['stub' => 'dto.stub']]);
        $this->files->shouldReceive('exists')->once()->withArgs(fn ($p) => str_contains($p, 'stubs/flowlight/dto.stub'))->andReturn(false);
        $this->files->shouldReceive('exists')->once()->withArgs(fn ($p) => str_contains($p, '/stubs/dto.stub'))->andReturn(true);
        $this->files->shouldReceive('get')->once()->withArgs(fn ($p) => str_contains($p, '/stubs/dto.stub'))->andReturn('// default dto stub');
    ...
    it('returns custom stub contents when custom override exists (real getConfig)', function () {
        ...
        $this->files->shouldReceive('exists')->once()->withArgs(fn ($p) => str_contains($p, 'stubs/flowlight/dto.stub'))->andReturn(true);
        $this->files->shouldReceive('get')->once()->withArgs(fn ($p) => str_contains($p, 'stubs/flowlight/dto.stub'))->andReturn('// custom dto stub');
    ...
    it('throws when neither custom nor default stub exists (real getConfig)', function () {
        ...
        $gen = makeApiGenRealProbe($this->files);
        $this->files->shouldReceive('exists')->once()->withArgs(fn ($p) => str_contains($p, 'stubs/flowlight/dto.stub'))->andReturn(false);
        $this->files->shouldReceive('exists')->once()->withArgs(fn ($p) => str_contains($p, '/stubs/dto.stub'))->andReturn(false);
